...
printlist:
	...
	beq $s0, $zero, endOfList
	lw $a0, 0($s0)		# print the value out
	li $v0, 1
	
	syscall 

			# prints the comma and space
	li $v0, 4
	la $a0, space
	syscall 
			
	addiu $s3, $s3, 1
	addiu $s0, $s0, NEXT_OFFSET
	addiu $v1, $s3, 0
	lw $t2, 0($s0)
	add $s0, $t2, $zero # find the address of the next word in the list and store it in s0 
	jal printlist
